...
namespace OldScatterTool
{
    ...
    {
        const string kSessionStateKeyPrefix = "SOS_";
        MyTreeView m_TreeView;
        SearchField m_SearchField;

        ItemAsset asset
        {
            get { return (ItemAsset)target; }
        }
        ...
        void OnEnable()
        {
            ...
            var treeModel = new TreeModel<ItemModel>(asset.items);
        ...
        void OnUndoRedoPerformed()
        {
            if (m_TreeView != null)
            {
                m_TreeView.treeModel.SetData(asset.items);
                m_TreeView.Reload();
            }
        }
        ...
        void ToolBar()
        {
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (GUILayout.Button("Add Item", style))
                {
                    ...
                    var element = new ItemModel("Item " + id, depth, id);
                ...
                if (GUILayout.Button("Remoooooove Item", style))
                {
        ...
        class MyTreeView : TreeViewWithTreeModel<ItemModel>
        {
            public MyTreeView(TreeViewState state, TreeModel<ItemModel> model) : base(state, model)
            {
                showBorder = true;
