...
//		sum += (fogLength >= dist) * (sum.a<0.99) * fgCol * (1.0-sum.a) * dir.w; // if fog hits geometry and accumulation is less than 0.99 add remaining fraction to reduce banding
	...
	#if defined(FOG_MASK)
	#define SET_DEPTH_MASK(uv, depth01) SetDepthMask(uv, depth01)
	inline void SetDepthMask(float2 uv, float depth01) {
		#if defined(FOG_ORTHO)
		...
		#endif
		_FogDistance.x = (depthMask * _ProjectionParams.z) * _FogData.w;
		_FogDistance.x *= _FogDistance.x;
		_FogDistance.y = 100.0;
	}
	#else
	#define SET_DEPTH_MASK(uv, depth01)
	#endif
